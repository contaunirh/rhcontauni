...
def montar_base(adm, dem, exames, epi, adt13, produtores):
    ...
    # ADT 13 / FERIAS
    adt_base = adt13.copy()
    adt_base.rename(columns={"TipoLancamento": "TipoExtra"}, inplace=True)
    ...
    adt_base["ValorADT13"] = adt_base.apply(lambda x: x["Valor"] if x["TipoExtra"] == "ADT13" else 0, axis=1)
    adt_base["Valor13"] = adt_base.apply(lambda x: x["Valor"] if x["TipoExtra"] == "13" else 0, axis=1)
    adt_base["ValorFerias"] = adt_base.apply(lambda x: x["Valor"] if x["TipoExtra"] == "Ferias" else 0, axis=1)
...
col4.metric("Multa FGTS R$", round(filtro["MultaFGTS"].sum() if "MultaFGTS" in filtro.columns else 0, 2))
col5.metric("Exames R$", round(filtro["ValorExame"].sum() if "ValorExame" in filtro.columns else 0, 2))
col6.metric("Uniformes e EPI R$", round(filtro["ValorEPI"].sum() if "ValorEPI" in filtro.columns else 0, 2))

col7, col8, col9 = st.columns(3)
col7.metric("Férias R$", round(filtro["ValorFerias"].sum() if "ValorFerias" in filtro.columns else 0, 2))
col8.metric("ADT13 R$", round(filtro["ValorADT13"].sum() if "ValorADT13" in filtro.columns else 0, 2))
col9.metric("13º R$", round(filtro["Valor13"].sum() if "Valor13" in filtro.columns else 0, 2))
